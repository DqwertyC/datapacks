...
# Check to see if we need to destroy a custom enchanting table (Table gone, or blocks placed around it)
...
execute as @e[type=item_frame,tag=enchanting_slot,nbt=!{ItemRotation:0b}] run data modify entity @s ItemRotation set value 0b
